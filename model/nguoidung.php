...
class NGUOIDUNG
{
	...
	{
		try {
			$this->db->beginTransaction(); // Bắt đầu transaction để tránh lỗi

			// Xóa tất cả địa chỉ liên quan trong `diachi`
			$sql1 = "DELETE FROM diachi WHERE nguoidung_id = :id";
			$stmt1 = $this->db->prepare($sql1);
			$stmt1->bindParam(":id", $id);
			$stmt1->execute();

			// Xóa người dùng sau khi đã xóa địa chỉ
			$sql2 = "DELETE FROM nguoidung WHERE id = :id";
			$stmt2 = $this->db->prepare($sql2);
			$stmt2->bindParam(":id", $id);
			$stmt2->execute();

			$this->db->commit(); // Xác nhận thay đổi
			return true;
		} catch (PDOException $e) {
			$this->db->rollBack(); // Hoàn tác nếu có lỗi
			echo "<p>Lỗi xóa người dùng: " . $e->getMessage() . "</p>";
			return false;
		}
	}
}
