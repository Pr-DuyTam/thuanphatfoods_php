...
switch ($action) {
    ...
    case "xoa":
        if (isset($_GET["mand"])) {
            if ($_SESSION["nguoidung"]["loai"] == 1) { // Chỉ Admin được xóa người dùng
                if ($nguoidungObj->xoanguoidung($_GET["mand"])) {
                    $tb = "Xóa người dùng thành công!";
                } else {
                    $tb = "Xóa người dùng thất bại!";
                }
            } else {
                $tb = "Bạn không có quyền xóa người dùng!";
            }
        }

        $nguoidung = $nguoidungObj->laydanhsachnguoidung();
        include("main.php");
        break;
